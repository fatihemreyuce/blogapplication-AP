...
import { AlertTriangle, MessageSquareText, ShieldAlert, Trash2, X } from "lucide-react";
...
export function DeleteCommentModal({
  ...
  const target = comment?.id.slice(0, 8) ?? "";
        ...
        <div className="relative bg-red-500/8 px-6 pb-5 pt-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Yorumu kalıcı olarak sileceksiniz.</p>
            </div>
          </div>
        </div>
        <form
        ...
        >
          <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/30 p-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold">Comment: {comment.id.slice(0, 8)}</p>
              <p className="truncate text-xs text-muted-foreground">{comment.content}</p>
            </div>
          </div>
          <div className="rounded-xl border border-red-500/15 bg-red-500/5 p-4">
            ...
            <p className="text-sm text-foreground/80">Silinen yorum geri yüklenemez.</p>
          </div>

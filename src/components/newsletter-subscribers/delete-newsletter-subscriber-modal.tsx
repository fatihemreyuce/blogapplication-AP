...
import { AlertTriangle, Mail, ShieldAlert, Trash2, X } from "lucide-react";
import type { NewsletterSubscriber } from "@/types/newsletter-subscriber.types";
import { cn } from "@/lib/utils";

interface DeleteNewsletterSubscriberModalProps {
  open: boolean;
...
export function DeleteNewsletterSubscriberModal({
  open,
  subscriber,
  loading = false,
  onConfirm,
  onCancel,
}: DeleteNewsletterSubscriberModalProps) {
  const [confirmText, setConfirmText] = useState("");
  const [shaking, setShaking] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  const target = subscriber?.email ?? "";
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-border/60 bg-card shadow-2xl">
        <div className="relative bg-red-500/8 px-6 pb-5 pt-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Abone kaydını kalıcı olarak sileceksiniz.</p>
            </div>
          </div>
        </div>
        <form
        ...
        >
          ...
          <div className="rounded-xl border border-red-500/15 bg-red-500/5 p-4">
            ...
            <p className="text-sm text-foreground/80">Silinen abonelik geri alınamaz.</p>
          </div>

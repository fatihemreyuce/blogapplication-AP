...
import { AlertTriangle, Heart, ShieldAlert, Trash2, X } from "lucide-react";
import type { Like } from "@/types/like.types";
import { makeLikeKey } from "@/types/like.types";
import { cn } from "@/lib/utils";

interface DeleteLikeModalProps {
  open: boolean;
  like: Like | null;
  loading?: boolean;
  onConfirm: () => void;
...
export function DeleteLikeModal({
  open,
  like,
  loading = false,
  onConfirm,
  onCancel,
}: DeleteLikeModalProps) {
  const [confirmText, setConfirmText] = useState("");
  const [shaking, setShaking] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  const target = like ? makeLikeKey(like.user_id, like.post_id).slice(0, 8) : "";
        ...
        <div className="relative bg-red-500/8 px-6 pb-5 pt-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Beğeni kaydını kalıcı olarak sileceksiniz.</p>
            </div>
          </div>
        </div>
        <form
        ...
        >
          <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/30 p-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold">{like.post?.title ?? like.post_id}</p>
          ...
          <div className="rounded-xl border border-red-500/15 bg-red-500/5 p-4">
            ...
            <p className="text-sm text-foreground/80">Silinen beğeni geri getirilemez.</p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3">
            <button type="button" onClick={onCancel} className="rounded-xl border border-border/60 px-4 py-2 text-sm">Vazgeç</button>

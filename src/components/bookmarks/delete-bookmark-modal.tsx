...
import { AlertTriangle, Bookmark, ShieldAlert, Trash2, X } from "lucide-react";
...
export function DeleteBookmarkModal({
  ...
  const target = bookmark ? makeBookmarkKey(bookmark.user_id, bookmark.post_id).slice(0, 8) : "";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-border/60 bg-card shadow-2xl">
        <div className="relative bg-red-500/8 px-6 pb-5 pt-6">
          ...
          <button onClick={onCancel} disabled={loading} className="absolute right-4 top-4 flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-accent">
            <X className="h-4 w-4" />
          </button>
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Bookmark kaydını kalıcı olarak sileceksiniz.</p>
            </div>
          </div>
        </div>
        <form
        ...
        >
          <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/30 p-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold">{makeBookmarkKey(bookmark.user_id, bookmark.post_id).slice(0, 8)}</p>
              <p className="truncate text-xs text-muted-foreground">{bookmark.post?.title ?? bookmark.post_id}</p>
            </div>
          </div>
          <div className="rounded-xl border border-red-500/15 bg-red-500/5 p-4">
            ...
            <p className="text-sm text-foreground/80">Bookmark kaydı geri getirilemez.</p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3">
            <button type="button" onClick={onCancel} className="rounded-xl border border-border/60 px-4 py-2 text-sm">Vazgec</button>
            <button type="submit" disabled={!isMatch || loading} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-red-500 to-red-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">

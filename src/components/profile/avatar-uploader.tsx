      ...
      if (uploadError) {
        throw new Error(
          uploadError.message.includes("Bucket not found")
            ? "Supabase Storage'da 'avatars' bucket'i bulunamadi. Public bir bucket olustur."
            : uploadError.message,
        );
      }
    ...
    <div className="space-y-2">
      ...
      >
        {isUploading ? (
          <>
            ...
            <p className="text-sm font-medium">Resim yukleniyor...</p>
          </>
        ) : (
          <>
            <Upload className="mb-2 h-5 w-5 text-muted-foreground" />
            <p className="text-sm font-medium">Resmi surukle birak ya da sec</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {value ? "Avatar yüklendi" : "Henuz avatar secilmedi"}
        </div>
        {value && (
          <Button
            type="button"

...
import { Hash, Plus, Search, Pencil, Trash2, Eye, ArrowUpDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useDeleteTag, useTagStats, useTags } from "@/hooks/use-tags";
...
function TagCard({
  ...
  return (
    <div
      className="animate-scale-in rounded-2xl border border-border/50 bg-card p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(59,130,246,0.12)]"
...
export default function TagListPage() {
  ...
  const { data, isLoading } = useTags({ search: searchParam, sort, page, pageSize: 12 });
    ...
    <div className="animate-fade-in space-y-6 p-1">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{stats?.total ?? 0} tag bulundu</p>
        </div>
        <Button onClick={() => navigate("/tags/create")} className="gap-2 border-0 text-white" style={{ background: "linear-gradient(135deg,#10b981,#3b82f6,#8b5cf6)" }}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[220px] flex-1">
          ...
          <Input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Tag ara..." className="pl-9" />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border/50 bg-card/50 py-20 text-center">
          <p className="text-lg font-semibold">Tag bulunamadi</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {data.data.map((tag, i) => (

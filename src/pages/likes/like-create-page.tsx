...
import { useCreateLike } from "@/hooks/use-like";
import { usePosts } from "@/hooks/use-post";
import { supabase } from "@/lib/supabase";

export default function LikeCreatePage() {
  const navigate = useNavigate();
  const createMutation = useCreateLike();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createMutation.mutateAsync({ user_id: currentUserId, post_id: form.post_id.trim() });
      toast.success("Beğeni oluşturuldu.");
      navigate("/likes");
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in space-y-6 p-1">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground">Yeni Beğeni Oluştur</h1>
        <p className="mt-1 text-sm text-muted-foreground">Gönderi seçerek beğeni kaydı ekleyin.</p>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-card">
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border/40 bg-muted/20 px-6 py-4 sm:px-8">
            <Button type="button" variant="outline" onClick={() => navigate("/likes")} disabled={isPending}>İptal</Button>

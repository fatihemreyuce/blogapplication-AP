...
export default function CommentCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Yorum olusturuldu.");
      navigate("/comments");
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in space-y-6 p-1">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground">Yeni Yorum Olustur</h1>
        <p className="mt-1 text-sm text-muted-foreground">Yorum alanlarini doldurarak kayit ekleyin.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-card">
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border/40 bg-muted/20 px-6 py-4 sm:px-8">
            <Button type="button" variant="outline" onClick={() => navigate("/comments")} disabled={isPending}>
              Iptal
            </Button>
            <Button
              type="submit"

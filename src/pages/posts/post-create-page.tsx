...
export default function PostCreatePage() {
  ...
  return (
    <div className="animate-fade-in space-y-6 p-1">
      <button onClick={() => navigate("/posts")} className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Posts</button>
      <div><h1 className="text-2xl font-bold">Yeni Post Oluştur</h1><p className="mt-1 text-sm text-muted-foreground">Post alanlarını doldurarak yeni içerik oluşturun.</p></div>
        ...
        <div className="overflow-hidden rounded-2xl border border-border/50 bg-card">
          ...
          <div className="space-y-5 p-6 sm:p-8">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5"><Label className="flex items-center gap-1.5"><FileText className="h-3.5 w-3.5 text-muted-foreground" />Başlık</Label><Input value={form.title} onChange={(e) => { const t = e.target.value; set("title")(t); if (!slugTouched) set("slug")(toSlug(t)); }} required disabled={isPending} /></div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border/40 bg-muted/20 px-6 py-4 sm:px-8">
            <Button type="button" variant="outline" onClick={() => navigate("/posts")} disabled={isPending}>İptal</Button>
            <Button type="submit" disabled={isPending} className="gap-2 border-0 text-white" style={{ background: "linear-gradient(135deg,#10b981,#3b82f6,#8b5cf6)" }}>
              {isPending ? <><Loader2 className="h-4 w-4 animate-spin" />Kaydediliyor...</> : <>Post Kaydet<ArrowRight className="h-4 w-4" /></>}
            </Button>
          </div>
        </div>
      </form>
